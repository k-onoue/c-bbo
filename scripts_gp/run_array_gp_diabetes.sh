#!/bin/bash -l
#SBATCH --job-name=gp_diabetes          # Job name
...
#SBATCH --output=logs/gp_diabetes_%A_%a.out
#SBATCH --error=logs/gp_diabetes_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_gp_diabetes.sh_snapshot"
fi

echo "Starting Diabetes GP experiment for n_paths=$N_PATHS_VAL, seed=$SEED at $TIMESTAMP..."
