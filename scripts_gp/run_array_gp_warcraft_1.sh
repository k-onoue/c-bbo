#!/bin/bash -l
#SBATCH --job-name=gp_warcraft_1        # Job name
...
#SBATCH --output=logs/gp_warcraft_1_%A_%a.out
#SBATCH --error=logs/gp_warcraft_1_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_gp_warcraft_1.sh_snapshot"
fi

echo "Starting Warcraft 1 GP experiment for n_paths=$N_PATHS_VAL, seed=$SEED at $TIMESTAMP..."
