...
# for src_file in "$TARGET_DIR"/run_*_train_${SRC_RANK}_*.sh; do
...
#   # 1. 新しいファイル名を生成
#   dest_file="${src_file/_train_${SRC_RANK}_/_train_${DEST_RANK}_}"
...
#   sed -i \
#     -e "s/_train_${SRC_RANK}_/_train_${DEST_RANK}_/g" \
#     -e "s/--tf_rank           ${SRC_RANK}/--tf_rank           ${DEST_RANK}/" \
#     -e "s/Train(Rank=${SRC_RANK})/Train(Rank=${DEST_RANK})/" \
#     "$dest_file"
...
for dest_rank in "${DEST_RANKS[@]}"; do
  ...
  for src_file in "$TARGET_DIR"/run_*_train_${SRC_RANK}_*.sh; do
    ...
    new_basename="${dest_file_basename/_train_${SRC_RANK}_/_train_${dest_rank}_}"
