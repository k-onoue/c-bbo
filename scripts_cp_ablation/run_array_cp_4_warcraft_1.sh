#!/bin/bash -l
#SBATCH --job-name=cp_2_warcraft_1   # Job name
...
#SBATCH --output=logs/cp_2_warcraft_1_%A_%a.out
#SBATCH --error=logs/cp_2_warcraft_1_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_cp_4_warcraft_1.sh_snapshot"
fi

echo "Starting Warcraft 1 CP(Rank=4) experiment for seed $SEED at $TIMESTAMP with lambda $LAMBDA_VAL..."
