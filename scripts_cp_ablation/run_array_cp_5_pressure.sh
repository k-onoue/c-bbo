#!/bin/bash -l
#SBATCH --job-name=cp_2_pressure     # Job name
...
#SBATCH --output=logs/cp_2_pressure_%A_%a.out
#SBATCH --error=logs/cp_2_pressure_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_cp_5_pressure.sh_snapshot"
fi

echo "Starting Pressure Vessel CP(Rank=5) experiment for seed $SEED at $TIMESTAMP with lambda $LAMBDA_VAL..."
