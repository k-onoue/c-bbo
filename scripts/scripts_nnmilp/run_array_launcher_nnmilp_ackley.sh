...
SAVEDIR="$PROJECT_ROOT/results_ackley_nmmilp_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/launcher_ackley.sh_snapshot"
...
for epochs in "${EPOCHS_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_nnmilp/run_array_nnmilp_ackley.sh" "$epochs" "$SHARED_TIMESTAMP" "$SAVEDIR"
