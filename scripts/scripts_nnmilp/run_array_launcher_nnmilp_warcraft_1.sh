...
SAVEDIR="$PROJECT_ROOT/results_warcraft_1_nnmilp_${LAUNCH_TIMESTAMP}"
...
for epochs in "${EPOCHS_VALUES[@]}"; do
  ...
  # 2. ディレクトリ名で使う共有タイムスタンプを、同じ未来の時刻で生成
  ...
  # 引数: <epochs_value> <shared_timestamp> <save_dir>
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_nnmilp/run_array_nnmilp_warcraft_1.sh" "$epochs" "$SHARED_TIMESTAMP" "$SAVEDIR"
