...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/launcher_pressure.sh_snapshot"
...
for epochs in "${EPOCHS_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_nnmilp/run_array_nnmilp_pressure.sh" "$epochs" "$SHARED_TIMESTAMP" "$SAVEDIR"
