...
SAVEDIR="$PROJECT_ROOT/results_warcraft_1_tpe_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/launcher_warcraft_1_tpe.sh_snapshot"
...
for n_paths in "${N_INIT_PATHS_VALUES[@]}"; do
  ...
  # sbatchで実験スクリプトを投入
  # 実行スクリプトは scripts_tpe/ ディレクトリにあると仮定
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_tpe/run_array_tpe_warcraft_1.sh" "$n_paths" "$SHARED_TIMESTAMP" "$SAVEDIR"
