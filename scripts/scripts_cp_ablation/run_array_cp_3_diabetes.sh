#!/bin/bash -l
#SBATCH --job-name=cp_2_diabetes     # Job name
...
#SBATCH --output=logs/cp_2_diabetes_%A_%a.out
#SBATCH --error=logs/cp_2_diabetes_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_cp_3_diabetes.sh_snapshot"
fi

echo "Starting Diabetes CP(Rank=3) experiment for seed $SEED at $TIMESTAMP with lambda $LAMBDA_VAL..."
