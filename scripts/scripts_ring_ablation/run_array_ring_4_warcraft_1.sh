#!/bin/bash -l
#SBATCH --job-name=ring_2_warcraft_1   # Job name
...
#SBATCH --output=logs/ring_2_warcraft_1_%A_%a.out
#SBATCH --error=logs/ring_2_warcraft_1_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_ring_4_warcraft_1.sh_snapshot"
fi

echo "Starting Warcraft 1 Ring(Rank=4) experiment for seed $SEED at $TIMESTAMP with lambda $LAMBDA_VAL..."
