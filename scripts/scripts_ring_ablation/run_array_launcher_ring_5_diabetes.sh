...
SAVEDIR="$PROJECT_ROOT/results_diabetes_ring_5_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/run_array_launcher_ring_5_diabetes.sh_snapshot"
...
for lambda in "${LAMBDA_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_ring/run_array_ring_5_diabetes.sh" "$lambda" "$SHARED_TIMESTAMP" "$SAVEDIR"
