#!/bin/bash -l
#SBATCH --job-name=ring_2_diabetes     # Job name
...
#SBATCH --output=logs/ring_2_diabetes_%A_%a.out
#SBATCH --error=logs/ring_2_diabetes_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_ring_6_diabetes.sh_snapshot"
fi

echo "Starting Diabetes Ring(Rank=6) experiment for seed $SEED at $TIMESTAMP with lambda $LAMBDA_VAL..."
