#!/bin/bash -l
#SBATCH --job-name=ring_2_ackley     # Job name
...
#SBATCH --output=logs/ring_2_ackley_%A_%a.out
#SBATCH --error=logs/ring_2_ackley_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_ring_5_ackley.sh_snapshot"
fi

echo "Starting Ackley Ring(Rank=5) experiment for seed $SEED at $TIMESTAMP with lambda $LAMBDA_VAL..."
