...
SAVEDIR="$PROJECT_ROOT/results_pressure_gp_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/launcher_pressure_gp.sh_snapshot"
...
for n_paths in "${N_INIT_PATHS_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_gp/run_array_gp_pressure.sh" "$n_paths" "$SHARED_TIMESTAMP" "$SAVEDIR"
