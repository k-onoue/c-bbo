...
SAVEDIR="$PROJECT_ROOT/results_warcraft_1_train_6_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/run_array_launcher_train_6_warcraft_1.sh_snapshot"
...
for lambda in "${LAMBDA_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_train/run_array_train_6_warcraft_1.sh" "$lambda" "$SHARED_TIMESTAMP" "$SAVEDIR"
