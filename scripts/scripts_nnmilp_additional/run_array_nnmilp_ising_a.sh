#!/bin/bash -l
#SBATCH --job-name=nnmilp_ising_a      # Job name
...
#SBATCH --output=logs/nnmilp_ising_a_%A_%a.out
#SBATCH --error=logs/nnmilp_ising_a_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_nnmilp_ising_a.sh_snapshot"
fi

echo "Starting Ising A NN+MILP experiment for seed $SEED at $TIMESTAMP with epochs = $EPOCHS_VAL..."

### --- Build argument array --- ###
N_TRIALS=500
HIDDEN_DIM=16
N_STARTUP=50
FUNCTION="ising_a"  # Change this to the desired function
