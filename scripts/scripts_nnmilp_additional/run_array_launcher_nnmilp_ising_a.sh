...
SAVEDIR="$PROJECT_ROOT/results_ising_a_nnmilp_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/launcher_ising_a.sh_snapshot"
...
for epochs in "${EPOCHS_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts_nnmilp_additional/run_array_nnmilp_ising_a.sh" "$epochs" "$SHARED_TIMESTAMP" "$SAVEDIR"
