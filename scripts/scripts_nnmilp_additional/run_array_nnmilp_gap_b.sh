#!/bin/bash -l
#SBATCH --job-name=nnmilp_gap_b    # Job name
...
#SBATCH --output=logs/nnmilp_gap_b_%A_%a.out
#SBATCH --error=logs/nnmilp_gap_b_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_nnmilp_gap_b.sh_snapshot"
fi

echo "Starting Gap B NN+MILP experiment for seed $SEED at $TIMESTAMP with epochs = $EPOCHS_VAL..."

### --- Build argument array --- ###
N_TRIALS=500
HIDDEN_DIM=16
N_STARTUP=50
FUNCTION="gap_b"  # Change this to the desired function
