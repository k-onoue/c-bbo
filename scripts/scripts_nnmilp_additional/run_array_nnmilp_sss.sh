#!/bin/bash -l
#SBATCH --job-name=nnmilp_sss    # Job name
...
#SBATCH --output=logs/nnmilp_sss%A_%a.out
#SBATCH --error=logs/nnmilp_sss%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_nnmilp_sss.sh_snapshot"
fi

echo "Starting SSS NN+MILP experiment for seed $SEED at $TIMESTAMP with epochs = $EPOCHS_VAL..."
...
HIDDEN_DIM=16
N_STARTUP=50
FUNCTION="sss"  # Change this to the desired function
