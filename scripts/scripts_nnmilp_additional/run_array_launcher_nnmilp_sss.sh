...
SAVEDIR="$PROJECT_ROOT/results_sss_nnmilp_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/launcher_sss.sh_snapshot"
...
for epochs in "${EPOCHS_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts_nnmilp_additional/run_array_nnmilp_sss.sh" "$epochs" "$SHARED_TIMESTAMP" "$SAVEDIR"
