...
SAVEDIR="$PROJECT_ROOT/results_pressure_train_4_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/run_array_launcher_train_4_pressure.sh_snapshot"
...
for lambda in "${LAMBDA_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_train/run_array_train_4_pressure.sh" "$lambda" "$SHARED_TIMESTAMP" "$SAVEDIR"
