...
SAVEDIR="$PROJECT_ROOT/results_ackley_ring_6_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/run_array_launcher_ring_6_ackley.sh_snapshot"
...
for lambda in "${LAMBDA_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_ring/run_array_ring_6_ackley.sh" "$lambda" "$SHARED_TIMESTAMP" "$SAVEDIR"
