...
SAVEDIR="$PROJECT_ROOT/results_diabetes_tpe_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/launcher_diabetes_tpe.sh_snapshot"
...
for n_paths in "${N_INIT_PATHS_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_tpe/run_array_tpe_diabetes.sh" "$n_paths" "$SHARED_TIMESTAMP" "$SAVEDIR"
