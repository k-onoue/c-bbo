...
SAVEDIR="$PROJECT_ROOT/results_warcraft_2_ring_3_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/run_array_launcher_ring_3_warcraft_2.sh_snapshot"
...
for lambda in "${LAMBDA_VALUES[@]}"; do
  ...
  # sbatchで実験スクリプトを投入
  # 実行スクリプトは scripts_ring/ ディレクトリにあると仮定
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts/scripts_ring/run_array_ring_3_warcraft_2.sh" "$lambda" "$SHARED_TIMESTAMP" "$SAVEDIR"
