#!/bin/bash -l
#SBATCH --job-name=train_2_ising_a     # Job name
...
#SBATCH --output=logs/train_2_ising_a_%A_%a.out
#SBATCH --error=logs/train_2_ising_a_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_train_2_ising_a.sh_snapshot"
fi

echo "Starting ISING-A Train(Rank=2) experiment for seed $SEED at $TIMESTAMP with lambda $LAMBDA_VAL..."
