#!/bin/bash -l
#SBATCH --job-name=train_4_ising_b     # Job name
...
#SBATCH --output=logs/train_4_ising_b_%A_%a.out
#SBATCH --error=logs/train_4_ising_b_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_train_4_ising_b.sh_snapshot"
fi

echo "Starting ISING-B Train(Rank=4) experiment for seed $SEED at $TIMESTAMP with lambda $LAMBDA_VAL..."
