#!/bin/bash -l
#SBATCH --job-name=nnmilp_ablation    # Job name
...
#SBATCH --output=logs/nnmilp_%A_%a.out
#SBATCH --error=logs/nnmilp_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_nnmilp.sh_snapshot"
fi

echo "Starting NN+MILP experiment for seed $SEED at $TIMESTAMP with epochs = $EPOCHS_VAL..."
