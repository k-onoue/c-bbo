#!/bin/bash -l
#SBATCH --job-name=nnmilp_warcraft    # Job name
...
#SBATCH --output=logs/nnmilp_warcraft_%A_%a.out
#SBATCH --error=logs/nnmilp_warcraft_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_nnmilp_warcraft.sh_snapshot"
fi

echo "Starting Warcraft NN+MILP experiment for seed $SEED at $TIMESTAMP with epochs = $EPOCHS_VAL..."
...
FUNCTION="warcraft"  # Change this to the desired function
