#!/bin/bash -l
#SBATCH --job-name=train_2_warcraft_1   # Job name
...
#SBATCH --output=logs/train_2_warcraft_1_%A_%a.out
#SBATCH --error=logs/train_2_warcraft_1_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_train_5_warcraft_1.sh_snapshot"
fi

echo "Starting Warcraft 1 Train(Rank=5) experiment for seed $SEED at $TIMESTAMP with lambda $LAMBDA_VAL..."
