...
# for src_file in "$TARGET_DIR"/run_*_cp_${SRC_RANK}_*.sh; do
...
#   # 1. 新しいファイル名を生成
#   dest_file="${src_file/_cp_${SRC_RANK}_/_cp_${DEST_RANK}_}"
...
#   sed -i \
#     -e "s/_cp_${SRC_RANK}_/_cp_${DEST_RANK}_/g" \
#     -e "s/--tf_rank           ${SRC_RANK}/--tf_rank           ${DEST_RANK}/" \
#     -e "s/CP(Rank=${SRC_RANK})/CP(Rank=${DEST_RANK})/" \
#     "$dest_file"
...
for dest_rank in "${DEST_RANKS[@]}"; do
  ...
  for src_file in "$TARGET_DIR"/run_*_cp_${SRC_RANK}_*.sh; do
    ...
    new_basename="${dest_file_basename/_cp_${SRC_RANK}_/_cp_${dest_rank}_}"
