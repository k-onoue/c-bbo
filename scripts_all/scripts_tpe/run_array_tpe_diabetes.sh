#!/bin/bash -l
#SBATCH --job-name=tpe_diabetes          # Job name
...
#SBATCH --output=logs/tpe_diabetes_%A_%a.out
#SBATCH --error=logs/tpe_diabetes_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_tpe_diabetes.sh_snapshot"
fi

echo "Starting Diabetes TPE experiment for n_paths=$N_PATHS_VAL, seed=$SEED at $TIMESTAMP..."
