...
SAVEDIR="$PROJECT_ROOT/results_diabetes_train_10_${LAUNCH_TIMESTAMP}"
...
cp "$SCRIPT_DIR/$(basename "$0")" "$SAVEDIR/run_array_launcher_train_10_diabetes.sh_snapshot"
...
for lambda in "${LAMBDA_VALUES[@]}"; do
  ...
  sbatch --begin="$BEGIN_TIME" \
         --chdir="$PROJECT_ROOT" \
         "$PROJECT_ROOT/scripts_train_additional/run_array_train_10_diabetes.sh" "$lambda" "$SHARED_TIMESTAMP" "$SAVEDIR"
