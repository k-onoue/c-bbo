#!/bin/bash -l
#SBATCH --job-name=train_12_diabetes     # Job name
...
#SBATCH --output=logs/train_12_diabetes_%A_%a.out
#SBATCH --error=logs/train_12_diabetes_%A_%a.err
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    cp "$0" "$TIMESTAMP_DIR/run_array_train_12_diabetes.sh_snapshot"
fi

echo "Starting Diabetes Train(Rank=12) experiment for seed $SEED at $TIMESTAMP with lambda $LAMBDA_VAL..."
